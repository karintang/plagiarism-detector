...
def load_xgboost_model(bucket, filepath, xgboost_model_name):
    """
    Gets trained and finetuned XGBoost model based on target S3 bucket and file path
    
    Args: 
        bucket (str): Name of S3 bucket.
        filepath (str): Filepath of trained XGBoost model.
        xgboost_model_name (str): File name of trained XGBoost model.
            
    Returns:
        model (XGBoost): Trained XGBoost model.
    ...
    s3client.download_file(
        Bucket = bucket,
        Key = filepath,
        Filename = xgboost_model_name
    )
    
    tar_file = tarfile.open(xgboost_model_name)
    tar_file.extractall('./model_folder')
    tar_file.close()
    xgboost_model = joblib.load('model_folder/xgboost_model.joblib')
...
def get_flag_score_prediction(xgboost_model_name, feature_df):
    """
    Returns the flag and probability predictions from the trained XGBoost model. 

    Args:
        xgboost_model_name (XGBoost): Trained XGBoost model.
        feature_df (pd.DataFrame): Dataframe of all features to be parsed to the trained XGBoost model.
    ...
    """
    xgboost_model = load_xgboost_model(s3_bucket, s3_folderpath+xgboost_model_name, xgboost_model_name)

    plagiarism_flag = xgboost_model.predict(feature_df)[0]
    plagiarism_score = xgboost_model.predict_proba(feature_df)[:,1][0]
    
    return plagiarism_flag, plagiarism_score

######## FINAL OUTPUT GENERATION FUNCTIONS ########

def one_one_matching(directory, s3_bucket, s3_folderpath, sentbert_model_name, xgboost_model_name, ngrams_lst, source_doc, source_name, input_doc, input_name):
    """
    ...
    Args:
        ...
        xgboost_model_name (str): Filepath of trained XGBoost model.
    ...
    plagiarism_flag, plagiarism_score = get_flag_score_prediction(xgboost_model_name, feature_df)
...
def get_matching_output(directory, s3_bucket, s3_folderpath, sentbert_model_name, xgboost_model_name, ngrams_lst, source_docs, input_doc, input_name):
    """
    ...
    for i in source_docs:
        temp_dict = one_one_matching(directory, s3_bucket, s3_folderpath, sentbert_model_name, xgboost_model_name, ngrams_lst, i['source_doc'], i['source_doc_name'], input_doc, input_name)
